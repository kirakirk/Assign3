...
void writeThatFile(FILE *diskImage, FILE *copyFile, int *SBI, char *name)
{
	...
	unsigned int *readSequence;	//an array to hold the sequence in the FAT
	
	readSequence = malloc(sizeof(unsigned int)*numBlocks);
...
//add the blocks of the file to the FAT
	...
	while(numBlocks2 > 1)
	{
		...
		readSequence[j+1] = htonl(readSequence[j+1]);
		m = fwrite(&readSequence[j+1], 4, 1, diskImage);
		readSequence[j+1] = ntohl(readSequence[j+1]);
		j++;
		numBlocks2--;
	}
...
//add the file info to the directory
	...
	//add the filename
	fwrite(name, 1, strlen(name), diskImage);
...
void readThatFile(FILE *diskImage, FILE *copyFile, int **allDirEntry, int *SBI)
{
	...
	if (allDirEntry[i][3] == 5)	//this is the file to print out
	{
		...
		unsigned char *toReadWrite;	//temp variable to hold bytes as they're read, before they're written
		
		int bytesLeft = fileSize;	//to keep track of the bytes left to read in the file
		unsigned int *readSequence;	//to store the sequence of blocks in the file system that the file is stored in, this is read from the FAT
